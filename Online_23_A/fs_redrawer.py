...
class FourierEpicycles:
    ...
    def prune_by_ratio(self, ratio):
        highest_scores = dict(sorted(self.coeffs.items(), key=lambda item: np.abs(item[1])**2, reverse=True))
        total = self.total_energy()
        sum = 0 
        picked = []
        for i in highest_scores.keys():
            value = highest_scores[i]
            value = np.abs(value)
            value = value**2
            sum = sum + value
            r = sum/total
        ...
        return len(picked), sum

    def MSE(self, pruned):
            sum = 0 
            for i in self.coeffs.keys():
                value = self.coeffs[i]
                value_2 = pruned.coeffs[i]
                value = np.abs(value- value_2)
                value = value**2
                sum = sum + value
            return sum/len(self.coeffs.keys())
    def copy(self):
        new = FourierEpicycles(self.t, self.signal, self.N)
        new.coeffs = dict(self.coeffs)
        return new
...
if __name__ == "__main__":
    ...
    ts = FourierEpicycles(t, z, n_harmonics=N_HARMONICS)
    ts.calculate_all_coefficients()
    ...
    r = np.array([0.96, 0.98, 1.00])
    for i in r :
        ts_1 = ts.copy()
        ratio = i 
        harmonics, energy = ts_1.prune_by_ratio(i)
        total = fs.total_energy()
        actual_ratio = energy/ total 
        mse = fs.MSE(ts_1)
        print(f"{ratio}         |             {harmonics}     |    {actual_ratio}   | {mse}") 
        save_outputs(ts_1, z, f"output/heart_pruned_{i}.png",f"output/heart_pruned_{i}.gif" , num_frames=240)
