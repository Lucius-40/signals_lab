...
class CFT2D:
    ...
    def compute_cft(self):
        ...
        #Integral(I(x,y)*cos(2pi ux))
        helper = Helper(self)
        g1_p = np.array([helper.g1_p_for_each_u(u) for u in self.u])
        #row = len(u=x) col = len(y)
        g1 = np.array([helper.g1_for_each_v(v,g1_p) for v in self.v])

        p1_p = np.array([helper.p1_p_for_each_u(u) for u in self.u])
                #row = len(u=x) col = len(y)
        p1 = np.array([helper.p1_for_each_v(v,p1_p) for v in self.v])

        real = g1 - p1 

        f1_p = p1_p.copy()
        f1 = np.array([helper.g1_for_each_v(v,f1_p) for v in self.v])

        k1_p = g1_p.copy()
        k1 = np.array([helper.p1_for_each_v(v,k1_p) for v in self.v])
...
class Helper : 
    ...
    def g1_p_for_each_u(self,u):
        I = self.cft2D.I
        integrand = I * np.cos(np.pi* 2 * self.cft2D.x*u)
        return np.trapezoid(integrand, self.cft2D.x, axis=1)
    def g1_for_each_v(self, v, integ):
        I=integ
        integrand = I * np.cos(2 * np.pi* self.cft2D.y*v)
        return np.trapezoid(integrand, self.cft2D.y, axis=1)

    def p1_p_for_each_u(self,u):
            I = self.cft2D.I
            integrand = I * np.sin(np.pi* 2 * self.cft2D.x*u)
            return np.trapezoid(integrand, self.cft2D.x, axis=1)
    def p1_for_each_v(self, v, integ):
            I=integ
            integrand = I * np.sin(2 * np.pi* self.cft2D.y*v)
            return np.trapezoid(integrand, self.cft2D.y, axis=1)
