...
def unit_phase(spectrum, epsilon=1e-12):
    ...
    result =  np.ones_like(spectrum, dtype=complex)
...
def swap_plane_spectra(plane_a, plane_b, engine):
    ...
    return result_ab.real, result_ba.real
...
def swap_images(image_a, image_b, engine):
    ...
    if image_a.ndim == 2:
        ...
        # return
        a, b = swap_plane_spectra(image_a,image_b, engine)
    ...
    if image_a.ndim == 3 and image_a.shape[2] == 3:
        ...
        # result_ba =
        # return
        pairs = [swap_plane_spectra(image_a[:,:,c],image_b[:,:,c],engine) for c in range(image_a.shape[2])]
        return np.dstack(pairs[0][0]), np.dstack(pairs[0][1])
