...
def choose_transform_shape(image_shape, kernel_shape, engine):
    ...
    if engine.name == "fft":
        ...
        # return
        adjusted_height = next_power_of_two(full_height)
        adjusted_width = next_power_of_two(full_width)
...
def hybrid_plane(low_plane, high_plane, kernel, engine):
    ...
    # full =
    result = inverse_2d(combined,engine)
    full = result.real
...
def hybrid_image(low_image, high_image, kernel, engine):
    ...
    if low_image.ndim == 3 and low_image.shape[2] == 3:
        ...
        # planes =
        # return
        planes = [hybrid_plane(low_image[:,:,c], high_image[::,c],kernel,engine ) for c in range(low_image.shape[2])]
